...
func handler(ctx context.Context, event protocol.BanSiteRequest) (protocol.GeneralResponse, error) {
	...
	if event.ShouldBan {
		...
		if err := redisCli.Set(fmt.Sprintf(config.BanSiteFormat, event.SiteId), "1", 0); err != nil {
			return protocol.GeneralResponse{Code: config.CodeBanSiteUpdateCacheError, Msg: fmt.Sprintf("%s banned", event.SiteId)}, err
		}
		...
		if err := redisCli.Del(fmt.Sprintf(config.BanSiteFormat, event.SiteId)); err != nil {
			return protocol.GeneralResponse{Code: config.CodeUnBanSiteUpdateCacheError, Msg: fmt.Sprintf("%s unbanned", event.SiteId)}, err
		}
