...
func handler(ctx context.Context, event protocol.BanUserRequest) (protocol.GeneralResponse, error) {
	...
	if event.ShouldBan {
		...
		if err := redisCli.Set(fmt.Sprintf(config.BanUserFormat, event.AnonymousId), "1", 0); err != nil {
			return protocol.GeneralResponse{Code: config.CodeBanUserUpdateCacheError, Msg: fmt.Sprintf("%s banned", event.AnonymousId)}, nil
		}
		...
		if err := redisCli.Del(fmt.Sprintf(config.BanUserFormat, event.AnonymousId)); err != nil {
			return protocol.GeneralResponse{Code: config.CodeUnBanUserUpdateCacheError, Msg: fmt.Sprintf("%s unbanned", event.AnonymousId)}, nil
		}
